...
public class ViveHold_SScipt : MonoBehaviour {
    ...
    void Update()
    {
            ...
            else //새로운 물체를 생성시켜 드래그 할 때
            {
                switch (target.tag)
                {
                    ...
                    case "MASSAPPLE":
                        ...
                        {
                            target = Instantiate(Resources.Load("poApple") as GameObject);
        ...
        if (controller.GetHairTriggerUp() && target != null)
        {
            if (target.GetComponent<Rigidbody>() != null)
            {
                ...
                rb.velocity = controller.velocity * 5f;
               rb.angularVelocity = controller.angularVelocity*2;
            }
